import type { AmbientOcclusionSpec, BoxPart, BoxPartColor, BuildingPlan, CompiledBuildingPlan, OpeningSpec, PlanePart, RoomSpec, SlabOpeningSpec, SurfaceSpec, Vec2, Vec3, WallSide } from './types'
...
// AO の幅だけは空間上の寸法なので plan.unit を反映する。
...
function compileRoomAmbientOcclusion(plan: BuildingPlan, room: RoomSpec): PlanePart[] {
  ...
  for (const side of sides) {
    ...
    if (ao.floor && room.surfaces?.floor?.hidden !== true) {
      for (const [index, segment] of segments.entries()) {
        ...
        if (!span) continue
        planes.push(createFloorWallAoPlane({
    ...
    if (ao.ceiling && room.surfaces?.ceiling?.hidden !== true) {
      for (const [index, segment] of segments.entries()) {
        ...
        const span = getWallContactAoSpan(segment, wallLength, wallThickness)
        if (!span) continue
        planes.push(createCeilingWallAoPlane({
    ...
    if (ao.openings) {
      ...
      for (const opening of localOpenings) {
        planes.push(...createOpeningAoPlanes({
          id: `${room.id}:ao:opening:${side}:${opening.id ?? opening.offset}`,
          side,
          roomCenter: [roomX, roomZ],
          roomSize: [roomWidth, roomDepth],
          wallThickness,
...
function createOpeningAoPlanes(input: Omit<AoWallInput, 'along'> & {
  opening: OpeningSpec & { splitAxis?: WallOpening['splitAxis'] }
  floorHeight: number
  slabThickness: number
}): PlanePart[] {
  const { opening, width, floorHeight, slabThickness } = input
  const start = opening.offset - opening.width / 2
  const end = opening.offset + opening.width / 2
  const bottom = opening.bottom ?? 0
  const top = bottom + (opening.height ?? OPENING_DEFAULTS.doorHeight)
  const visibleBottom = Math.max(bottom, slabThickness)
  const visibleTop = Math.min(top, floorHeight - slabThickness)
  const height = visibleTop - visibleBottom
  const planes: PlanePart[] = []

  if (height > EPSILON) {
    const leftFace = getInteriorWallFacePosition({ ...input, along: start - width / 2 })
    const rightFace = getInteriorWallFacePosition({ ...input, along: end + width / 2 })
    const leftReverse = isWallPlaneLocalXAlignedWithWallAlong(input.side)
    const wallRotation = getWallAoRotation(input.side)

    planes.push(
      {
        id: `${input.id}:left`, kind: 'ambientOcclusion', position: [leftFace[0], (visibleBottom + visibleTop) / 2, leftFace[2]],
        size: [width, height], rotation: wallRotation, gradientAxis: 'x', gradientReverse: leftReverse,
        strength: input.strength, falloff: input.falloff,
      },
      {
        id: `${input.id}:right`, kind: 'ambientOcclusion', position: [rightFace[0], (visibleBottom + visibleTop) / 2, rightFace[2]],
        size: [width, height], rotation: wallRotation, gradientAxis: 'x', gradientReverse: !leftReverse,
        strength: input.strength, falloff: input.falloff,
      },
    )
  ...
  const topBandHeight = Math.min(width, floorHeight - slabThickness - top)
  if (topBandHeight > EPSILON) {
    const face = getInteriorWallFacePosition({ ...input, along: opening.offset })
    planes.push({
      id: `${input.id}:top`, kind: 'ambientOcclusion', position: [face[0], top + topBandHeight / 2, face[2]],
      size: [opening.width, topBandHeight], rotation: getWallAoRotation(input.side), gradientAxis: 'y',
      strength: input.strength, falloff: input.falloff,
    })
  }

  // ドア下辺は床-壁 AO と重なるため省略し、床から浮いた窓だけ下辺を描く。
  const bottomBandHeight = Math.min(width, bottom - slabThickness)
  if (bottomBandHeight > EPSILON) {
    const face = getInteriorWallFacePosition({ ...input, along: opening.offset })
    planes.push({
      id: `${input.id}:bottom`, kind: 'ambientOcclusion', position: [face[0], bottom - bottomBandHeight / 2, face[2]],
      size: [opening.width, bottomBandHeight], rotation: getWallAoRotation(input.side), gradientAxis: 'y', gradientReverse: true,
      strength: input.strength, falloff: input.falloff,
    })
  }
