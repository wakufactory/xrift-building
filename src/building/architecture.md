...
このワールドは、`src/worldPlan.tsx` にある `BuildingPlan` データを `BuildingWorld` に渡し、汎用 building レイヤーで構造用の `BoxPart[]` と視覚 effect 用の `PlanePart[]` に変換して描画と物理へ分配します。建物以外の家具・外構 box は `BoxInstance[]` として `BoxLayer` に渡せます。物理が必要な場合は `BoxLayer collider` を指定します。
...
flowchart TD
  ...
  E --> F["Building<br/>plan を box / plane に compile"]
  F --> G["compileBuildingRenderPlan(plan)<br/>BuildingPlan を BoxPart[] / PlanePart[] に変換"]
  F --> H["BoxLayer collider<br/>Provider 配下では描画登録と collider 生成"]
  F --> P["InstancedPlaneLayer<br/>AO plane を instanced 描画"]
...
中心になる中間表現は `BoxInstance`、`BoxPart`、`PlanePart` です。`BoxInstance` は描画・物理共通の汎用 box で、家具や外構にも使います。`BoxPart` は `BoxInstance` に建物内の分類 `kind` を足した建物コンパイル結果です。`PlanePart` は AO や壁紙などの表面 effect 用で、構造・collider には使いません。建物 compiler が生成する床、外部地面、壁、天井、柱は基本的に axis-aligned box として表現されますが、描画と物理の経路自体は `rotation` 付き box も扱えます。
...
- `ambientOcclusion?: AmbientOcclusionSpec | false`
  - 接合部に疑似 AO plane を生成します。未指定時は生成しません。
  - `width` は `unit` によって world units へスケールされます。`strength`、`falloff`、対象種別は描画設定なのでスケールしません。
- `pillar?: PillarSpec`
...
- `ambientOcclusion?: AmbientOcclusionSpec | false`
  - 建物全体の AO 設定を部屋単位で上書きします。`false` はこの部屋の AO を完全に無効化します。
...
`Building` は `compileBuildingRenderPlan(plan)` の `boxes` を `BoxLayer` に、`planes` を `InstancedPlaneLayer` に渡します。互換用の `compileBuildingPlan(plan)` は引き続き `BoxPart[]` だけを返します。
...
`renderInstances` は構造 box の描画 instance 数、`planeInstances` は `InstancedPlaneLayer` の描画 instance 数、`totalRenderInstances` は両者の合計です。現在の plane kind は `ambientOcclusion` です。
