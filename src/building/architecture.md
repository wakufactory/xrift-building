...
このワールドは、`src/worldPlan.tsx` にある `BuildingPlan` データを `BuildingWorld` に渡し、汎用 building レイヤーで `BoxPart[]` に変換して描画と物理へ分配します。建物以外の家具・外構 box は `BoxInstance[]` として直接 `BoxLayer` / `BoxColliders` に渡せます。
...
flowchart TD
  ...
  A --> L["BoxColliders<br/>追加 box の物理"]
  ...
  F --> H["BoxLayer<br/>Provider 配下では登録だけ行う"]
  F --> I["BoxColliders<br/>BoxInstance ごとに CuboidCollider 化"]
  H --> B
...
- `src/World.tsx`
  ...
  - 建物以外のサンプル box `exteriorBoxes` を `BoxLayer` / `BoxColliders` に渡します。
...
- `src/building/`
  ...
  - `BoxBatchProvider` / `BoxLayer` / `BoxColliders` により、建物以外の box 描画・物理にも使える低レベル API を持ちます。
...
`Building` は同じ `BoxPart[]` を `BoxColliders` に渡します。
...
建物自体の拡張は、まず `BuildingPlan` の入力表現を増やし、最終的に `BoxPart[]` に落とす形を保つのがよいです。家具やエクステリアのように建物 plan に入れないものは、`BoxInstance[]` と共通 material catalog を直接 `BoxLayer` / `BoxColliders` に渡します。
